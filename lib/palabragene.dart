...
class AprenderPalabra extends StatefulWidget {
  final int idUsuario;
  final int idModulo;

  const AprenderPalabra({
    Key? key,
  ...
  @override
  State<AprenderPalabra> createState() => _AprenderPalabraState();
}

class _AprenderPalabraState extends State<AprenderPalabra> {
  List<String> listaPalabras = [
    'Gato', 'Perro', 'Casa', 'Escuela', 'Flor', 'Cielo', 'Arbol',
    'Silla', 'Libro', 'Abuela', 'Abuelo', 'Mesa', 'Bañera', 'Barco', 'Bota',
  ...
  void _generarPalabra() {
    setState(() {
      palabraActual = listaPalabras[indiceActual];
    });
  }
  ...
  Future<void> _decirPalabra() async {
    await _configurarTTS();
    await flutterTts.speak(palabraActual);
  }

  Future<void> _decirObjeto() async {
    await _configurarTTS();
    String objeto = nombresObjetos[palabraActual] ?? '';
    await flutterTts.speak("$palabraActual, es como $objeto");
  }

  Future<void> _decirFinal() async {
    await _configurarTTS();
    await flutterTts.speak(
      "Has finalizado el modulo. Si quieres continuar, presiona el boton verde. Y si quieres reiniciar la leccion, presiona el boton naranja.",
    );
  }
  ...
  Widget build(BuildContext context) {
    String imagenObjeto = 'imagen/fotos/${palabraActual.toLowerCase()}.png';
